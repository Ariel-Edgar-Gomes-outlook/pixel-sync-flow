-- Final post-migration permissions and storage policies required after restoring the schema
...
-- Contract signing is public via signature token, so the visitor must be able to upload the signature image.
-- Keep all private owner-folder policies intact; only allow the specific public signatures/ folder in this bucket.
